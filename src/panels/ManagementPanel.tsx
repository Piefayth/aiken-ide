...
function ManagementPanel() {
    const buildStatus = useSelector((state: RootState) => state.project.buildResult)

    const numWarnings = buildStatus?.warnings.length
    const numErrors = buildStatus?.errors.length
    const numTests = buildStatus?.test_results.length
    const numFailedTests = buildStatus?.test_results.filter(test => !test.success).length || 0
    ...
    const testsSegment = !numTests || numTests <= 0 ? null : (
        <div className="build-results-section tests">
            ...
            {
                buildStatus?.test_results.map((test) => {
                    return (
